  ...
  <v-container fluid>
    ...
    <v-row class="pt-4">
      <v-col align-self="end" ref="macroTaskContainer">
        <diagram
          ref="macroTaskDiagram"
          v-on:taskSelected="selectedTask"
          :dataset="dataset"
          :station="station"
          :recomendations="recomendations"
        />
...
export default {
  ...
  props: ['dataset', 'station', 'recomendations'],
  watch: {
    dataset: function(newValue, oldValue) {
      this.seePresentChart = false
      console.log('Dataset cambiado', newValue, oldValue, this.station)
      axios
  ...
  methods: {
    ...
    async disgregateData(raw_data) {
      ...
      if (
        ...
        keys.forEach(async key => {
          let feature_points = await raw_data.map(day_data => ({
            label: day_data.date,
            y: day_data[key]
          }))
          resData.push({
    ...
    async newOperation() {
      ...
      if (this.operation.operation_name === 'normalize') {
        req_string = `http://localhost:8080/op/${this.dataset}/normalize`
        this.history.push({
          name: 'scale',
          aux: req_string
        })
      } else if (this.operation.operation_name === 'reduce') {
        req_string = `http://localhost:8080/op/${this.dataset}/reduce/${this.operation.n_comp}`
        this.history.push({
          name: 'reduce',
          aux: req_string
        })
        ...
        req_string = `http://localhost:8080/op/${this.dataset}/transform/${this.operation.factor}`
        this.history.push({
          name: 'reduce',
          aux: req_string
        })
        ...
        req_string = `http://localhost:8080/op/${this.dataset}`
      }
  ...
  mounted: function() {
    console.log('Time series', this.dataset, this.station, this.recomendations)
    ...
    console.log('Time series re:', this.recomendations)
